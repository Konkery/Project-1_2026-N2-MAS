...
4. **Справочники (Handbooks)** — нормализованные реестры НСИ (проекты, контрагенты, бренды, классификатор единиц измерения).
...
├── 01__КБД/                                 # Корпоративная База Данных (BI-датасеты и скрипты)
│   ├── 01__SQL/                             # Реляционные базы данных, дампы и SQL-скрипты
│   ├── 02__NoSQL/                           # Неструктурированные и документоориентированные БД
│   └── 03__Excel/                           # Нормализованные Excel-справочники 1-го типа (Handbook)
...
│       └── TableList/                       # Центральный реестр таблиц-справочников (TabTableList)
...
> **Связь с ETL-процессором:** Запросы нормативно-справочной информации (`HandbookFunction`, `UtilsGetPathTable`) обращаются напрямую к каталогу `c:\OneDrive\01__LLC_MAS_2025\КИТ\01__КБД\03__Excel\TableList\` для автоматического резолвинга путей к мастер-таблицам.
...
**Дата актуализации:** 10 сентября 2026 г.