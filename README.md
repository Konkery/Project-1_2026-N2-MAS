...
flowchart TD
    ...
    subgraph CoreEngine ["ETL-ядро экосистемы"]
        Processor["ETL-процессор ProcessingTable<br/>(версия — по исходному коду)"]:::etl
...
**`ProcessingTable` (актуальная версия — по исходному коду ядра и документации)** — параметризованная M-функция, считывающая конфигурационный JSON из таблицы `TabConfigJSON` текущей книги по ключу `_configJSONIndex` и выполняющая сквозной конвейер обработки данных для 1–4 таблиц Excel.
...
### 2. Конвейер языка «HorizonBI»

Внутри каждого нумерованного МакроБлока (`"1"`, `"2"`...) команды выполняются в строгом системном порядке приоритетов ядра. Неизменные правила конвейера: ключи секций — только числа, исполнение по возрастанию (`"2"` раньше `"10"`); одна команда — не более одного вызова в МакроБлоке; конфигурация содержит от 1 до 4 таблиц и ровно одну команду слияния (только в Таблице 1).
...
**Версия документации:** 2.2 (Синхронизировано с ядром `rev.04 v03` и шаблоном `rev.04 v03`)  
