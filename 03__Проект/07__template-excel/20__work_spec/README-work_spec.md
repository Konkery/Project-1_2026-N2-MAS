...
|  2  | `IDsource`                 | int   | [1, 300]      |  ❌  |       ❌        | Внешний идентификатор позиции в ИД (допускает NULL)       |
...
| 11  | `HASH COLLECTION`          | text  | [6, 10]       |  ✅  |       ❌        | **Глобальный ключ связи спецификаций**                    |
...
| 18  | `UNITS`                    | text  | [0, 2]        |  ❌  |       ❌        | Код единицы измерения (выпадающий список `HandbookUnit`)  |
...
| 23  | `PRICE UNIT $PURCHASE`     | float | [1, 5000000]  |  ❌  |    🔒 **ДА**    | **Цена закупки за ед. БЕЗ НДС** (чистая база закупки)     |
| 24  | `VAT ADD PRICE $PURCHASE`  | float | [0, 2000000]  |  ❌  |    🔒 **ДА**    | **Входящий НДС на ед. закупки** (ручной ввод / 0 при УСН) |
| 25  | `COST PRICE $PURCHASE`     | float | [1, 10000000] |  ✅  |    🔒 **ДА**    | Себестоимость партии закупки БЕЗ НДС (`21 * 23`)          |
| 26  | `COST PRICE VAT $PURCHASE` | float | [1, 10000000] |  ✅  |    🔒 **ДА**    | **Себестоимость закупки С НДС** (`25 + 21 * 24`)          |
| 27  | `TCP SOURCE`               | int   | [7, 1000]     |  ❌  |    🔒 **ДА**    | Идентификатор ТКП поставщика (`HandbookTCP`)              |
...
| 30  | `IMAGE PRODUCT`            | text  | [7, 1000]     |  ❌  |       ❌        | Имя файла изображения ТМЦ (допускает NULL)                |
...
|  №  | Имя столбца                  | Тип   | Диапазон      | Авто | Конфиденциально | Описание                                                            |
| :-: | :--------------------------- | :---- | :------------ | :--: | :-------------: | :------------------------------------------------------------------ |
|  1  | `HASH COLLECTION`            | text  | [6, 10]       |  ✅  |       ❌        | **Ключ связи с таблицей Work_1**                                    |
...
| 18  | `COMMENT CUSTOMER`           | text  | [0, 3000]     |  ❌  |       ❌        | Публичное примечание для заказчика в ТКП (допускает NULL)           |
