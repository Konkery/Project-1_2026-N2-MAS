...
- **Дата последнего обновления**: 14 сентября 2026 г.
...
| Лист                  | Тип                  | Назначение                                                                  |
| :-------------------- | :------------------- | :-------------------------------------------------------------------------- |
| `SpecificationWork_1` | Рабочий              | Основная таблица Master Data (техника, закупки, логистика: 29 колонок)      |
...
| `TableSchema`         | Служебный            | Таблица `TabTableSchema` (схема данных и правила валидации 47 колонок)      |
...
TabSpecificationWork_2 (18 колонок)
         ...
         │
         ↓
TabSpecificationWork (46 колонок без дубля ключа) → Pivot → Аналитика
```
...
Таблица **`TabTableSchema`** расположена на листе `TableSchema` и содержит полную машиночитаемую спецификацию метаданных для всех **47 столбцов** спецификации.
...
- **Всего описанных столбцов:** 47 (29 для `Work_1` + 18 для `Work_2`)
...
2. `NUM COLUMN` — порядковый номер столбца (1..29 и 1..18).
...
## 5. СПЕЦИФИКАЦИЯ TabSpecificationWork_1 (29 колонок)
...
| 26  | `TCP SOURCE`              | text  | [7, 1000]    |  ❌  |    🔒 **ДА**    | Ссылка на ТКП поставщика (`HandbookTCP`)                  |
| 27  | `COMMENT WORK`            | text  | [7, 3000]    |  ❌  |    🔒 **ДА**    | Служебный комментарий инженера                            |
| 28  | `URL PRODUCT`             | text  | [7, 1000]    |  ❌  |       ❌        | Ссылка на официальную карточку товара                     |
| 29  | `IMAGE PRODUCT`           | text  | [7, 1000]    |  ❌  |       ❌        | Имя или путь к файлу изображения ТМЦ                      |
...
2.  **Категория 2 (Строго автоматические):** Колонки `NUM ORDER`, `NUM FULL SET`, `HASH COLLECTION`, `COST PRICE $PURCHASE`, расчетные налоговые столбцы НДС (`Col_8`, `Col_10`, `Col_11`, `Col_13`, `Col_14` в `Work_2`). Изменение формул запрещено.
