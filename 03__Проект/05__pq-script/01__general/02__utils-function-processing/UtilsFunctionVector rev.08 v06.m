let
    ...
    // ---------------------------------------------------------------------------------------------
    // Функция трансформации 'ftFinancialRound'
    // Выполняет банковское (бухгалтерское) округление для комплекса связанных финансовых полей, 
    // обеспечивая абсолютный математический баланс: Итого = Цена * Количество.
    // 
    // Функция рассчитывает и ПЕРЕЗАПИСЫВАЕТ столбцы: Итоговая стоимость (без НДС), 
    // Итоговый НДС, Итоговая стоимость (с НДС), пересчитанная Цена за ед. (без НДС), Цена за ед. (с НДС).
    //
    // аргументы:
    // _sourceTable as table      - таблица (объект Power Query), переданная из процессора
    // _configList as list        - конфигурация параметров
    //                              [SourceQuantity, SourceBasePrice, SourceVatRate, 
    //                               TargetPriceNoVat, TargetCostNoVat, TargetVatSum, 
    //                               TargetPriceVat, TargetCostVat]
    //
    // возвращаемое значение:
    // table                      - таблица со сбалансированными и округленными значениями
    // ---------------------------------------------------------------------------------------------
    ftFinancialRound = (
        _sourceTable as table,
        _configList as list
    ) as table =>
    let
        // Извлекаем имена столбцов из конфигурации (строгий порядок)
        SourceQuantity   = _configList{0},
        SourceBasePrice  = _configList{1},
        SourceVatRate    = _configList{2},
        TargetPriceNoVat = _configList{3},
        TargetCostNoVat  = _configList{4},
        TargetVatSum     = _configList{5},
        TargetPriceVat   = _configList{6},
        TargetCostVat    = _configList{7},

        // Проверяем наличие всех исходных столбцов
        requiredInputCols = {SourceQuantity, SourceBasePrice, SourceVatRate},
        missingInputCols = List.Select(requiredInputCols, each not Table.HasColumns(_sourceTable, {_})),
        validatedInputTable = if List.Count(missingInputCols) > 0 then 
            error "ftFinancialRound: Отсутствуют исходные столбцы: " & Text.Combine(missingInputCols, ", ") 
        else _sourceTable,

        // Проверяем наличие целевых столбцов (функция Modify-типа, столбцы уже должны быть в таблице,
        // либо мы добавляем их, но в контексте ProcessingTable они обычно есть или переименованы)
        requiredTargetCols = {TargetPriceNoVat, TargetCostNoVat, TargetVatSum, TargetPriceVat, TargetCostVat},
        missingTargetCols = List.Select(requiredTargetCols, each not Table.HasColumns(validatedInputTable, {_})),
        validatedTable = if List.Count(missingTargetCols) > 0 then
             error "ftFinancialRound: Отсутствуют целевые столбцы для перезаписи: " & Text.Combine(missingTargetCols, ", ")
        else validatedInputTable,

        // Функция банковского округления до 2 знаков
        RoundBankers = (val as number) as number => Number.Round(val, 2, RoundingMode.ToEven),

        // Выполняем построчный пересчет
        TransformedTable = Table.ReplaceRows(validatedTable, 0, Table.RowCount(validatedTable),
            List.Transform(Table.ToRecords(validatedTable), (row) =>
                let
                    // 1. Исходные значения
                    qty = try Number.From(Record.Field(row, SourceQuantity)) otherwise 0,
                    basePrice = try Number.From(Record.Field(row, SourceBasePrice)) otherwise 0,
                    vatRate = try Number.From(Record.Field(row, SourceVatRate)) otherwise 0, // например 0.22

                    // 2. Расчет базовой стоимости
                    costExact = basePrice * qty,
                    costRounded = RoundBankers(costExact),

                    // 3. Расчет НДС от базовой стоимости
                    vatSumExact = costRounded * vatRate,
                    vatSumRounded = RoundBankers(vatSumExact),

                    // 4. Расчет итоговой стоимости с НДС
                    costVatRounded = costRounded + vatSumRounded,

                    // 5. Обратный пересчет цены
                    priceNoVatRounded = if qty <> 0 then RoundBankers(costRounded / qty) else 0,
                    priceVatRounded = if qty <> 0 then RoundBankers(costVatRounded / qty) else 0,

                    // 6. Формирование новой строки с перезаписью полей
                    updatedRow = Record.TransformFields(row, {
                        {TargetPriceNoVat, each priceNoVatRounded},
                        {TargetCostNoVat,  each costRounded},
                        {TargetVatSum,     each vatSumRounded},
                        {TargetPriceVat,   each priceVatRounded},
                        {TargetCostVat,    each costVatRounded}
                    })
                in
                    updatedRow
            )
    ...
    // ---------------------------------------------------------------------------------------------
    // Функция трансформации 'ftGetMaxSpecList' (Специализированная для спецификаций HorizonBI)
    // Группирует строки таблицы по столбцу-группе (например 'NUM FULL SET'), внутри каждой группы
    // находит строку с МАКСИМАЛЬНЫМ значением в столбце-критерии (например 'VER COLLECTION') и
    // возвращает список значений столбца-результата (например 'HASH COLLECTION') из найденных строк.
    //
    // Назначение в бизнес-логике: реализация паттерна "Latest Version" - отбор актуальных
    // (старших по версии) позиций технической спецификации 'work spec' для дальнейшей фильтрации
    // с помощью 'ColumnFilterByList' или ручного 'Table.SelectRows'.
    //
    // аргументы:
    // _sourceTable as table       - таблица (объект Power Query), в которой производится поиск
    // _groupColumn as text        - имя столбца группировки (например 'NUM FULL SET')
    // _indexSearchColumn as text  - имя столбца, по которому ищется максимальное значение (например 'VER COLLECTION')
    // _returnColumn as text       - имя столбца, значение которого необходимо вернуть (например 'HASH COLLECTION')
    //
    // возвращаемое значение:
    // list                        - список значений '_returnColumn' для строк с максимальным '_indexSearchColumn' в каждой группе
    // ---------------------------------------------------------------------------------------------
    ftGetMaxSpecList = (
        _sourceTable as table,
        _groupColumn as text,
        _indexSearchColumn as text,
        _returnColumn as text
    ) as list =>
    let
        requiredCols = {_groupColumn, _indexSearchColumn, _returnColumn},
        missingCols = List.Select(requiredCols, each not Table.HasColumns(_sourceTable, {_})),
        validatedTable = if List.Count(missingCols) > 0 then
            error "ftGetMaxSpecList: В таблице отсутствуют столбцы: " & Text.Combine(missingCols, ", ")
        else _sourceTable,

        GroupedTable = Table.Group(validatedTable, {_groupColumn}, {
            {"__BestRow__", (groupRows) => 
                let
                    sorted = Table.Sort(groupRows, {{_indexSearchColumn, Order.Descending}})
                in
                    sorted{0}, type record}
        }),
        result = List.Transform(GroupedTable[__BestRow__], each Record.Field(_, _returnColumn))
    in
        result,

    // ---------------------------------------------------------------------------------------------
    // Функция трансформации 'ftGetMinSpecList' (Специализированная для спецификаций HorizonBI)
    // Полный аналог 'ftGetMaxSpecList', но отбирает строку с МИНИМАЛЬНЫМ значением критерия
    // в каждой группе (например, для отбора самой ранней/первой версии позиции).
    //
    // аргументы:
    // _sourceTable as table       - таблица (объект Power Query), в которой производится поиск
    // _groupColumn as text        - имя столбца группировки (например 'NUM FULL SET')
    // _indexSearchColumn as text  - имя столбца, по которому ищется минимальное значение (например 'VER COLLECTION')
    // _returnColumn as text       - имя столбца, значение которого необходимо вернуть (например 'HASH COLLECTION')
    //
    // возвращаемое значение:
    // list                        - список значений '_returnColumn' для строк с минимальным '_indexSearchColumn' в каждой группе
    // ---------------------------------------------------------------------------------------------
    ftGetMinSpecList = (
        _sourceTable as table,
        _groupColumn as text,
    ...
    let
        ...
        missingCols = List.Select(requiredCols, each not Table.HasColumns(_sourceTable, {_})),
        validatedTable = if List.Count(missingCols) > 0 then
            error "ftGetMinSpecList: В таблице отсутствуют столбцы: " & Text.Combine(missingCols, ", ")
        else _sourceTable,

        GroupedTable = Table.Group(validatedTable, {_groupColumn}, {
            {"__BestRow__", (groupRows) => 
                let
                    sorted = Table.Sort(groupRows, {{_indexSearchColumn, Order.Ascending}})
                in
                    sorted{0}, type record}
        }),
        result = List.Transform(GroupedTable[__BestRow__], each Record.Field(_, _returnColumn))
    ...
    // ---------------------------------------------------------------------------------------------
    // Функция трансформации 'ftGetMaxValue' (Универсальная BI-функция)
    // Возвращает максимальное числовое значение из указанного столбца таблицы. При передаче
    // необязательного параметра '_groupColumn' возвращает таблицу с максимумом внутри каждой группы
    // (аналог агрегатной функции 'MAX(...) GROUP BY ...' в классическом BI/SQL-анализе).
    //
    // аргументы:
    // _sourceTable as table          - таблица (объект Power Query)
    // _columnName as text            - имя столбца, в котором ищется максимальное значение
    // optional _groupColumn as text  - имя столбца группировки (если не указан - поиск по всей таблице)
    //
    // возвращаемое значение:
    // any                            - число (максимум по всей таблице) либо table (максимум по группам:
    //                                  столбцы '_groupColumn' и 'MAX_VALUE')
    // ---------------------------------------------------------------------------------------------
    ftGetMaxValue = (
        _sourceTable as table,
        _columnName as text,
        optional _groupColumn as text
    ) as any =>
    let
        validatedTable = if not Table.HasColumns(_sourceTable, {_columnName}) then
            error "ftGetMaxValue: В таблице отсутствует столбец '" & _columnName & "'."
        else if _groupColumn <> null and not Table.HasColumns(_sourceTable, {_groupColumn}) then
            error "ftGetMaxValue: В таблице отсутствует столбец группировки '" & _groupColumn & "'."
        else _sourceTable,

        result = if _groupColumn = null then
            List.Max(Table.Column(validatedTable, _columnName))
        else
            Table.Group(validatedTable, {_groupColumn}, {
                {"MAX_VALUE", each List.Max(Table.Column(_, _columnName)), type number}
            })
    in
        result,

    // ---------------------------------------------------------------------------------------------
    // Функция трансформации 'ftGetMinValue' (Универсальная BI-функция)
    // Полный аналог 'ftGetMaxValue', но возвращает минимальное числовое значение (по всей таблице
    // либо по группам).
    //
    // аргументы:
    // _sourceTable as table          - таблица (объект Power Query)
    // _columnName as text            - имя столбца, в котором ищется минимальное значение
    // optional _groupColumn as text  - имя столбца группировки (если не указан - поиск по всей таблице)
    //
    // возвращаемое значение:
    // any                            - число (минимум по всей таблице) либо table (минимум по группам:
    //                                  столбцы '_groupColumn' и 'MIN_VALUE')
    // ---------------------------------------------------------------------------------------------
    ftGetMinValue = (
        _sourceTable as table,
        _columnName as text,
    ...
    let
        validatedTable = if not Table.HasColumns(_sourceTable, {_columnName}) then
            error "ftGetMinValue: В таблице отсутствует столбец '" & _columnName & "'."
        else if _groupColumn <> null and not Table.HasColumns(_sourceTable, {_groupColumn}) then
            error "ftGetMinValue: В таблице отсутствует столбец группировки '" & _groupColumn & "'."
        else _sourceTable,

        result = if _groupColumn = null then
            List.Min(Table.Column(validatedTable, _columnName))
        else
            Table.Group(validatedTable, {_groupColumn}, {
                {"MIN_VALUE", each List.Min(Table.Column(_, _columnName)), type number}
            })
    in
        result,
...
// =============================================================================================
// ЭКСПОРТНАЯ БАЗА ВСЕХ ФУНКЦИЙ В RECORD
// =============================================================================================
FunctionRecord = [
    ...
    ftFinancialRound        = ftFinancialRound,         // 10
    ftGetMaxSpecList        = ftGetMaxSpecList,         // 11
    ftGetMinSpecList        = ftGetMinSpecList,         // 12
    ftGetMaxValue           = ftGetMaxValue,            // 13
    ftGetMinValue           = ftGetMinValue,            // 14
    
    // Функции валидации
    fvCheckBoolean          = fvCheckBoolean,           // 15
    fvCheckDate             = fvCheckDate,              // 16
    fvCheckDateRange        = fvCheckDateRange,         // 17
    fvCheckEmail            = fvCheckEmail,             // 18
    fvCheckFloat            = fvCheckFloat,             // 19
    fvCheckINN_LLC          = fvCheckINN_LLC,           // 20
    fvCheckINN_Personal     = fvCheckINN_Personal,      // 21
    fvCheckSNILS            = fvCheckSNILS,             // 22
    fvCheckInteger          = fvCheckInteger,           // 23
    fvCheckKPP              = fvCheckKPP,               // 24
    fvCheckNonEmpty         = fvCheckNonEmpty,          // 25
    fvCheckIntegerRange     = fvCheckIntegerRange,      // 26
    fvCheckFloatRange       = fvCheckFloatRange,        // 27
    fvCheckPhone            = fvCheckPhone,             // 28
    fvCheckTextLength       = fvCheckTextLength,        // 29
    fvCheckPassportNumber   = fvCheckPassportNumber,    // 30
    fvCheckPassportSeries   = fvCheckPassportSeries,    // 31
    fvCheckPassportUnitCode = fvCheckPassportUnitCode,  // 32
    fvCheckID               = fvCheckID,                // 33
    fvCheckID_2             = fvCheckID_2               // 34
]
