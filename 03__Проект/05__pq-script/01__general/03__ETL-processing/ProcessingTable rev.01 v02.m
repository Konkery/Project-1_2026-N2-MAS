...
let
    ...
    CheckUniqueness = if Table.RowCount(MatchingConfigRow) = 0 then
        ...
        error "В таблице '" & ConfigTableName & "' найдено несколько конфигураций с ID '" & _configJSONIndex & "'."
    else
        MatchingConfigRow,
        ...
        let
            ...
            Result = if List.IsEmpty(ClearColRules) then TableAfterRemoveCol else
                let
                    ...
                    transformations = if List.Count(missingCols) > 0 then 
                                          error "ClearColumn: В таблице отсутствуют столбцы для очистки: " & Text.Combine(missingCols, ", ")
                                      else
                                          List.Transform(ClearColRules, (colName) => {colName, each null})
                in
        ...
        let
            ...
            Result = if List.IsEmpty(ReorderColRules) then TableAfterRename else
                let
                    ...
                    // Формируем итоговый порядок: сначала столбцы из JSON, затем остальные
                ...
                in
                    if List.Count(missingCols) > 0 then
                        error "ReorderColumn: В таблице отсутствуют столбцы для переупорядочивания: " & Text.Combine(missingCols, ", ")
                    else
                        Table.ReorderColumns(TableAfterRename, finalOrder)
        in
            Result,
