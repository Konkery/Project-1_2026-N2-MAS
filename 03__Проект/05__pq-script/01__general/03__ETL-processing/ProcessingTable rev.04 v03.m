...
let
    ...
    ConfigValidation = if TableCount > 4 then error "Конфигурация не может содержать более 4 таблиц." else
        let
            ...
            ForbiddenCommands = JoinCommands,
        ...
        in
            ...
            else if List.IsEmpty(FoundForbiddenInOthers) = false then error "Команды слияния (Run...JOIN...) могут быть указаны только в секциях ПЕРВОЙ таблицы в JSON."
            else "OK",

    // =============================================================================================
    // ВСПОМОГАТЕЛЬНАЯ ФУНКЦИЯ: ДВИЖОК ВАЛИДАЦИИ ДАННЫХ (RunValidationEngine)
    // =============================================================================================
    // Универсальный движок валидации, вызываемый как этап конвейера МакроБлока (ValidationData),
    // так и в финальной секции. Формирует служебный столбец 'VALIDATION' со строкой кодов ошибок.
    // Если в правилах присутствует { "StrictBlock": true }, при обнаружении любой ошибки
    // выполнение прерывается критической ошибкой 'error' с перечнем проблемных столбцов и строк.
    // ---------------------------------------------------------------------------------------------
    RunValidationEngine = (_sourceTable as table, _validationRulesList as list) as table =>
        let
            // Извлекаем флаг строгой блокировки StrictBlock (если присутствует среди правил).
            StrictRules = List.Select(_validationRulesList, each Record.HasFields(_, "StrictBlock")),
            IsStrictBlock = if List.IsEmpty(StrictRules) then false else Record.Field(StrictRules{0}, "StrictBlock"),
            // Оставляем только правила проверки столбцов (исключаем служебный StrictBlock).
            ColumnRulesOnly = List.Select(_validationRulesList, each not Record.HasFields(_, "StrictBlock")),
            CleanedTable = if Table.HasColumns(_sourceTable, {"VALIDATION"}) then Table.RemoveColumns(_sourceTable, {"VALIDATION"}) else _sourceTable,
            BufferedSourceTable = Table.Buffer(CleanedTable),
            ColumnRulesRecord = if List.IsEmpty(ColumnRulesOnly) then [] else Record.Combine(ColumnRulesOnly),
            ...
            ResultTableWithValidation = Table.RemoveColumns(AddValidationColumn, {"__Index__"}),
            // Проверка строгой блокировки: ищем строки, где VALIDATION содержит ненулевой код ошибки.
            InvalidRows = if not IsStrictBlock then {} else Table.SelectRows(ResultTableWithValidation, each List.AnyTrue(List.Transform(Text.Split([VALIDATION], "-"), (code) => try Number.FromText(code) > 0 otherwise false))),
            FinalResult = if IsStrictBlock and Table.RowCount(InvalidRows) > 0 then
                error "ValidationData (StrictBlock): Обнаружены критические ошибки валидации в " & Text.From(Table.RowCount(InvalidRows)) & " строк(е/ах) исходного датасета. Генерация производной спецификации остановлена. Проверьте обязательные поля и корректность значений в спецификации 'work spec' и повторите валидацию."
                else ResultTableWithValidation
        in
            FinalResult,
        ...
        let
            ...
            ProcessedTable = List.Accumulate(SortedMacroBlockNames, InitialSourceTable, (tableState, macroBlockName) =>
                let
                    ...
                    // Этап SortData: многоуровневая сортировка (Ascending/Descending) перед дедупликацией.
                    TableAfterSort = let SortRules = Record.FieldOrDefault(RulesRecord, "SortData", {}), Result = if List.IsEmpty(SortRules) then TableAfterFilterFree else let currentCols = Table.ColumnNames(TableAfterFilterFree), sortSpec = List.Transform(SortRules, (rule) => let colName = Record.FieldNames(rule){0}, dirValue = Record.Field(rule, colName), pqOrder = if dirValue = "Ascending" then Order.Ascending else if dirValue = "Descending" then Order.Descending else error "SortData: Указано неизвестное направление сортировки '" & Text.From(dirValue) & "' для столбца '" & colName & "'. Допустимо 'Ascending' или 'Descending'." in {colName, pqOrder}), sortCols = List.Transform(sortSpec, each _{0}), missingCols = List.Select(sortCols, each not List.Contains(currentCols, _)) in if List.Count(missingCols) > 0 then error "SortData: В таблице отсутствуют столбцы для сортировки: " & Text.Combine(missingCols, ", ") else Table.Sort(TableAfterFilterFree, sortSpec) in Result,
                    TableAfterFilterUnique = let FilterRules = Record.FieldOrDefault(RulesRecord, "ColumnFilterUniqueData", {}), Result = if List.IsEmpty(FilterRules) then TableAfterSort else List.Accumulate(FilterRules, TableAfterSort, (subTableState, columnName) => let validatedTable = if not Table.HasColumns(subTableState, {columnName}) then error "ColumnFilterUniqueData: В таблице отсутствует столбец для фильтрации: " & columnName else subTableState in Table.Distinct(validatedTable, {columnName})) in Result,
                    ...
                    TableAfterFinancialRound = let FinRoundRules = Record.FieldOrDefault(RulesRecord, "FinancialRound", {}), Result = if List.IsEmpty(FinRoundRules) then TableAfterModify else List.Accumulate(FinRoundRules, TableAfterModify, (subTableState, currentRule) => let ruleRecord = Record.Field(currentRule, Record.FieldNames(currentRule){0}), configList = {ruleRecord[SourceQuantity], ruleRecord[SourceBasePrice], ruleRecord[SourceVatRate], ruleRecord[TargetPriceNoVat], ruleRecord[TargetCostNoVat], ruleRecord[TargetVatSum], ruleRecord[TargetPriceVat], ruleRecord[TargetCostVat]} in UtilsFunctionVector[ftFinancialRound](subTableState, configList)) in Result,
                    TableAfterRemoveXor = let KeepColRules = Record.FieldOrDefault(RulesRecord, "RemoveXorColumn", {}), Result = if List.IsEmpty(KeepColRules) then TableAfterFinancialRound else let currentCols = Table.ColumnNames(TableAfterFinancialRound), missingCols = List.Select(KeepColRules, each not List.Contains(currentCols, _)), validatedTable = if List.Count(missingCols) > 0 then error "RemoveXorColumn: В таблице отсутствуют столбцы для сохранения: " & Text.Combine(missingCols, ", ") else TableAfterFinancialRound in Table.SelectColumns(validatedTable, KeepColRules) in Result,
                    ...
                    // Этап ValidationData: универсальная валидация датасета внутри МакроБлока.
                    // Формирует служебный столбец 'VALIDATION' с кодами ошибок (0 - валидно, N - номер столбца с ошибкой).
                    // При наличии параметра "StrictBlock": true конвейер прерывается критической ошибкой при первой невалидной строке.
                    TableAfterValidation = let ValidationRulesList = Record.FieldOrDefault(RulesRecord, "ValidationData", {}), Result = if List.IsEmpty(ValidationRulesList) then TableAfterReorder else RunValidationEngine(TableAfterReorder, ValidationRulesList) in Result
                in
                    TableAfterValidation
            )
        in
            ProcessedTable
    ),
    ...
    // =============================================================================================
    // ШАГ 6: ВОЗВРАТ ИТОГОВОГО ДАТАСЕТА
    // =============================================================================================
    // Начиная с версии rev.04 v03, команда 'ValidationData' является полноправным этапом конвейера
    // МакроБлоков (см. ШАГ 3) и может вызываться в любой точке обработки (в т.ч. первой командой
    // с флагом "StrictBlock": true для жесткой пред-валидации, и/или последней командой как
    // генератор служебного столбца 'VALIDATION'). Отдельная финальная секция валидации упразднена.
    
    FinalTable = TableAfterJoin
in
    FinalTable