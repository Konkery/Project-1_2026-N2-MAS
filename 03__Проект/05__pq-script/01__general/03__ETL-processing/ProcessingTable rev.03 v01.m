...
// 'TabConfigJSON', на закладке 'ConfigJSON'.
//
// аргументы:
...
let
    ...
    // Извлекаем запись со всеми МакроБлоками
    ...
    // =============================================================================================
    // ШАГ 3: ПОСЛЕДОВАТЕЛЬНОЕ ВЫПОЛНЕНИЕ МАКРОБЛОКОВ
    // =============================================================================================
    
    // Получаем имена (номера) всех МакроБлоков
    MacroBlockNames = Record.FieldNames(MacroBlocksRecord),
    // Сортируем их как числа по возрастанию, чтобы гарантировать порядок выполнения 1, 2, 3...
    SortedMacroBlockNames = List.Sort(MacroBlockNames, (a, b) => Value.Compare(Number.FromText(a), Number.FromText(b))),

    // Используем List.Accumulate для последовательного выполнения каждого МакроБлока
    ProcessedTable = List.Accumulate(SortedMacroBlockNames, InitialSourceTable, (tableState, macroBlockName) =>
        let
            // Извлекаем запись с правилами для ТЕКУЩЕГО МакроБлока
            RulesRecord = Record.Field(MacroBlocksRecord, macroBlockName),

            // -- ЭТАП 1: ФИЛЬТРАЦИЯ ПО ПРОИЗВОЛЬНЫМ ЗНАЧЕНИЯМ --
            TableAfterFilterFree =
                let
                    FilterRules = Record.FieldOrDefault(RulesRecord, "ColumnFilterFreeData", {}),
                    Result = if List.IsEmpty(FilterRules) then tableState else
                        List.Accumulate(FilterRules, tableState, (subTableState, currentRule) =>
                            let
            ...
            // -- ЭТАП 2: ФИЛЬТРАЦИЯ ПО УНИКАЛЬНЫМ ЗНАЧЕНИЯМ --
            TableAfterFilterUnique = 
                let
                    FilterRules = Record.FieldOrDefault(RulesRecord, "ColumnFilterUniqueData", {}),
                    Result = if List.IsEmpty(FilterRules) then TableAfterFilterFree else
                        List.Accumulate(FilterRules, TableAfterFilterFree, (subTableState, columnName) =>
                            let
            ...
            TableAfterInsert =
                let
                    ...
                    Result = if List.IsEmpty(InsertRules) then TableAfterFilterUnique else
                        List.Accumulate(InsertRules, TableAfterFilterUnique, (subTableState, newColumnName) =>
