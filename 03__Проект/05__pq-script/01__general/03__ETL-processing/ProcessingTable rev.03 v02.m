...
// ---------------------------------------------------------------------------------------------
// Процессор 'ProcessingTable' выполняет комплексную обработку исходной таблицы Excel, заданной
// пользователем, на основе правил описанных в JSON конфигурации, хранящейся в таблице
// 'TabConfigJSON'.
//
// аргументы:
...
let
    ...
    // =============================================================================================
    // ШАГ 3: ПРОВЕРКА СТРУКТУРЫ МАКРОБЛОКОВ
    // =============================================================================================
    
    MacroBlockNames = Record.FieldNames(MacroBlocksRecord),
    CheckMacroBlockKeys = List.AllTrue(List.Transform(MacroBlockNames, each (try Number.FromText(_) otherwise null) <> null)),
    ValidationMacroBlocks = if not CheckMacroBlockKeys then error "Ошибка конфигурации: Имена МакроБлоков (секций) в JSON должны быть только числами (например, ""1"", ""2"", ...)." else "OK",

    CheckUniqueCommands = List.Accumulate(MacroBlockNames, "OK", (state, blockName) =>
        let
            commands = Record.FieldNames(Record.Field(MacroBlocksRecord, blockName)),
            distinctCommands = List.Distinct(commands)
        in
            if List.Count(commands) <> List.Count(distinctCommands) then
                error "Ошибка конфигурации: В МакроБлоке (секции) """ & blockName & """ обнаружены повторяющиеся BI-команды."
            else
                state
    ),

    // =============================================================================================
    // ШАГ 4: ПОСЛЕДОВАТЕЛЬНОЕ ВЫПОЛНЕНИЕ МАКРОБЛОКОВ
    // =============================================================================================
    ...
    ProcessedTable = List.Accumulate(SortedMacroBlockNames, InitialSourceTable, (tableState, macroBlockName) =>
        let
            ...
            // -- ЭТАП 1: УСТАНОВКА ТИПОВ ДАННЫХ ('ColumnSetType') --
            TableAfterSetType =
                let
                    SetTypeRulesList = Record.FieldOrDefault(RulesRecord, "ColumnSetType", {}),
                    Result = if List.IsEmpty(SetTypeRulesList) then tableState else
                        let
                            ...
                            currentCols = Table.ColumnNames(tableState),
                        ...
                        in
                            if List.Count(missingCols) > 0 then
                                ...
                                Table.TransformColumnTypes(tableState, TypeTransformations)
                in
                    Result,

            // -- ЭТАП 2: ФИЛЬТРАЦИЯ ПО УНИКАЛЬНЫМ ЗНАЧЕНИЯМ ('ColumnFilterUniqueData') --
            TableAfterFilterUnique = 
                let
                    FilterRules = Record.FieldOrDefault(RulesRecord, "ColumnFilterUniqueData", {}),
                    Result = if List.IsEmpty(FilterRules) then TableAfterSetType else
                        List.Accumulate(FilterRules, TableAfterSetType, (subTableState, columnName) =>
                            let
                                validatedTable = if not Table.HasColumns(subTableState, {columnName}) then error "ColumnFilterUniqueData: В таблице отсутствует столбец для фильтрации: " & columnName else subTableState
                            in
            ...
            // -- ЭТАП 3: ФИЛЬТРАЦИЯ ПО ПРОИЗВОЛЬНЫМ ЗНАЧЕНИЯМ ('ColumnFilterFreeData') --
            TableAfterFilterFree =
                let
                    ...
                    Result = if List.IsEmpty(FilterRules) then TableAfterFilterUnique else
                        List.Accumulate(FilterRules, TableAfterFilterUnique, (subTableState, currentRule) =>
                            let
                                colName = Record.FieldNames(currentRule){0},
                                filterValue = Record.Field(currentRule, colName),
                                validatedTable = if not Table.HasColumns(subTableState, {colName}) then error "ColumnFilterFreeData: В таблице отсутствует столбец для фильтрации: " & colName else subTableState
                            in
            ...
            // -- ЭТАП 4: УДАЛЕНИЕ ОДНОЙ СТРОКИ ('RemoveRowOne') --
            TableAfterRemoveOneRow = 
                let
                    RemoveRules = Record.FieldOrDefault(RulesRecord, "RemoveRowOne", {}),
                    Result = if List.IsEmpty(RemoveRules) then TableAfterFilterFree else
                        List.Accumulate(RemoveRules, TableAfterFilterFree, (subTableState, currentRule) =>
                            let
                                ...
                                columnAsTextList = List.Transform(Table.Column(subTableState, searchCol), each Text.From(_)),
                                rowIndex = List.PositionOf(columnAsTextList, searchVal)
                            in
                                if rowIndex = -1 then error "RemoveRowOne: Не удалось найти строку, где '" & searchCol & "' = '" & searchVal & "'." else Table.RemoveRows(subTableState, rowIndex, 1)
                        )
                in
                    Result,

            // -- ЭТАП 5: УДАЛЕНИЕ ВСЕХ НАЙДЕННЫХ СТРОК ('RemoveRowAll') --
            TableAfterRemoveAllRows =
                let
                    RemoveRules = Record.FieldOrDefault(RulesRecord, "RemoveRowAll", {}),
                    Result = if List.IsEmpty(RemoveRules) then TableAfterRemoveOneRow else
                        List.Accumulate(RemoveRules, TableAfterRemoveOneRow, (subTableState, currentRule) =>
                            let
                                ruleRecord = Record.Field(currentRule, Record.FieldNames(currentRule){0}),
                                searchCol = ruleRecord[IndexSearchColumn],
                                searchVal = Text.From(ruleRecord[IndexSearchRow])
                            in
                                Table.SelectRows(subTableState, each Text.From(Record.Field(_, searchCol)) <> searchVal)
                        )
                in
                    Result,
            
            // -- ЭТАП 6: ВСТАВКА НОВЫХ СТОЛБЦОВ ('InsertColumn') --
            TableAfterInsert =
                let
                    InsertRules = Record.FieldOrDefault(RulesRecord, "InsertColumn", {}),
                    Result = if List.IsEmpty(InsertRules) then TableAfterRemoveAllRows else
                        List.Accumulate(InsertRules, TableAfterRemoveAllRows, (subTableState, newColumnName) =>
                            if Table.HasColumns(subTableState, {newColumnName}) then
                                error "InsertColumn: Столбец с именем '" & newColumnName & "' уже существует в таблице."
                            else
                                Table.AddColumn(subTableState, newColumnName, each null)
                        )
                in
                    Result,
            
            // -- ЭТАП 7: ЗАПОЛНЕНИЕ СТОЛБЦОВ ('ColumnFillData') --
            TableAfterFill =
                let
                    FillRules = Record.FieldOrDefault(RulesRecord, "ColumnFillData", {}),
                    Result = if List.IsEmpty(FillRules) then TableAfterInsert else
                        List.Accumulate(FillRules, TableAfterInsert, (subTableState, currentRule) =>
                            let
                                colName = Record.FieldNames(currentRule){0},
                                fillValue = Record.Field(currentRule, colName),
                                validatedTable = if not Table.HasColumns(subTableState, {colName}) then error "ColumnFillData: В таблице отсутствует столбец для заполнения: " & colName else subTableState
                            in
                    ...
                    Result = if List.IsEmpty(ModifyRules) then TableAfterFill else
                        List.Accumulate(ModifyRules, TableAfterFill, (subTableState, currentRule) =>
                            let
                                ...
                                columnAsTextList = List.Transform(Table.Column(subTableState, searchCol), each Text.From(_)),
                                rowIndex = List.PositionOf(columnAsTextList, searchVal),
                                FinalTable = if rowIndex = -1 then error "ModifyData: Не удалось найти строку, где '" & searchCol & "' = '" & searchVal & "'." else let oldRowRecord = subTableState{rowIndex}, newRowRecord = Record.Combine({oldRowRecord, newData}) in Table.ReplaceRows(subTableState, rowIndex, 1, {newRowRecord})
                            in FinalTable)
                in
                    Result,

            // -- ЭТАП 9: УДАЛЕНИЕ СТОЛБЦОВ (XOR) ('RemoveXorColumn') --
            TableAfterRemoveXor =
                let
                    KeepColRules = Record.FieldOrDefault(RulesRecord, "RemoveXorColumn", {}),
                    Result = if List.IsEmpty(KeepColRules) then TableAfterModify else
                        let
                            currentCols = Table.ColumnNames(TableAfterModify),
                            missingCols = List.Select(KeepColRules, each not List.Contains(currentCols, _)),
                            validatedTable = if List.Count(missingCols) > 0 then error "RemoveXorColumn: В таблице отсутствуют столбцы для сохранения: " & Text.Combine(missingCols, ", ") else TableAfterModify
                        in
                            Table.SelectColumns(validatedTable, KeepColRules)
                in
                    Result,
            
            // -- ЭТАП 10: УДАЛЕНИЕ СТОЛБЦОВ ('RemoveColumn') --
            TableAfterRemoveCol = 
                let
            ...
            // -- ЭТАП 11: ОЧИСТКА СТОЛБЦОВ ('ClearColumn') --
            TableAfterClearCol =
                let
            ...
            // -- ЭТАП 12: ПЕРЕИМЕНОВАНИЕ СТОЛБЦОВ ('RenameColumn') --
            TableAfterRename =
                let
            ...
            // -- ЭТАП 13: ПЕРЕУПОРЯДОЧИВАНИЕ СТОЛБЦОВ ('ReorderColumn') --
            TableAfterReorder =
                let
            ...
            // -- ЭТАП 14: ВАЛИДАЦИЯ ДАННЫХ ('ValidationData') --
            TableWithValidation = if not Record.HasFields(RulesRecord, "ValidationData") then
                TableAfterReorder
            else
                let
