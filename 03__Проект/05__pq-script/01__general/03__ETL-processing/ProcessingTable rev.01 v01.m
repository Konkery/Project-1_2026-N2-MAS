...
let
    ...
    // Определяем источник для ИСХОДНЫХ ДАННЫХ на основе аргумента _pathExcelBook
    SourceWorkbook = if _pathExcelBook = null or _pathExcelBook = "" then
                         Excel.CurrentWorkbook() 
                     else
                         Excel.Workbook(File.Contents(_pathExcelBook)), 

    // --- ИСПРАВЛЕНИЕ: Корректный доступ к данным в зависимости от источника ---
    // Загружаем целевую таблицу из определенного источника
    InitialSourceTable = 
        let
            Source = if _pathExcelBook = null or _pathExcelBook = "" then
                // Для текущей книги получаем доступ к столбцу [Content]
                try SourceWorkbook{[Name=TargetTableName]}[Content] otherwise null
            else
                // Для внешней книги получаем доступ к столбцу [Data]
                ...
                List.Accumulate(ModifyRules, InitialSourceTable, (tableState, currentRule) =>
                    let
                        ...
                        FinalTable = if rowIndex = -1 then 
                                        error "ModifyData: Не удалось найти строку, где '" & searchCol & "' = '" & Text.From(searchVal) & "'." 
                                     else
                                        let oldRowRecord = tableState{rowIndex}, newRowRecord = Record.Combine({oldRowRecord, newData}) in Table.ReplaceRows(tableState, rowIndex, 1, {newRowRecord})
                    in FinalTable)
        in
            Result,
    ...
    // -- ЭТАП 4: ПЕРЕИМЕНОВАНИЕ СТОЛБЦОВ --
    TableAfterRename =
        let
            ...
            Result = if List.IsEmpty(transformations) then TableAfterRemoveCol else
                let
                    ...
                    validatedTable = if List.Count(missingCols) > 0 then error "RenameColumn: В таблице отсутствуют столбцы для переименования: " & Text.Combine(missingCols, ", ") else TableAfterRemoveCol
                in
    ...
    // =============================================================================================
    // ШАГ 4 И 5: ВАЛИДАЦИЯ И ПРИМЕНЕНИЕ ТИПОВ (ВЫПОЛНЯЮТСЯ ТОЛЬКО ПРИ НАЛИЧИИ БЛОКА 'ValidationData')
    // =============================================================================================
    
    FinalTable = if not Record.HasFields(RulesRecord, "ValidationData") then
        // ЕСЛИ БЛОКА 'ValidationData' НЕТ, ПРОСТО ВОЗВРАЩАЕМ РЕЗУЛЬТАТ ТРАНСФОРМАЦИЙ
        TableAfterRename
    else
        // ЕСЛИ БЛОК 'ValidationData' ЕСТЬ, ЗАПУСКАЕМ ПОЛНУЮ ЛОГИКУ ВАЛИДАЦИИ
        let
            // --- ИСПРАВЛЕНИЕ: Блок очистки VALIDATION перенесен сюда ---
            CleanedTable = if Table.HasColumns(TableAfterRename, {"VALIDATION"}) then
                               Table.RemoveColumns(TableAfterRename, {"VALIDATION"})
                           else
                               TableAfterRename,
            ...
            PreValidationResults =
                let
                    ...
                    ResultsRecord = List.Accumulate(IdColumns, [], (state, currentColumn) =>
                        let
                            // Передаем путь к файлу в fvCheckID
                            errorList = QueryUtilsFunction[fvCheckID](TargetTableName, currentColumn, _pathExcelBook)
                        in
            ...
            result = if List.IsEmpty(FinalTypeTransformations) then
                        TableWithValidation
                     else
                        Table.TransformColumnTypes(TableWithValidation, FinalTypeTransformations)
        in
            result
in
    FinalTable